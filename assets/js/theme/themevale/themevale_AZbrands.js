...
export default class AZBrands {
    ...
    updateAtoZBrands($brands, $atozBrands) {
        ...
        setTimeout(function(){ 
            $('#azBrands .azBrands-group').each(function(e){
                var check = $(this).find('> .brandList').text().trim();
                var t_aztxt = $(this).data('letter');
                if(check == ''){
                    $('#azBrandsTable [data-letter="'+t_aztxt+'"]').find('a').addClass('disable');
                }
                else {
                    $('#azBrandsTable [data-letter="'+t_aztxt+'"]').find('a').removeClass('disable');
                }
            });
        }, 1000);
    }
