...
import additionalProductColors from '../themevale/themevale_additionalProductColors';
import swal from '../global/sweet-alert';

export default class ProductDetailsQuickShop {
    ...
    getViewModel($scope) {
        return {
            ...
            $priceQS: $('.productView-price', $scope),
        };
    }
    ...
    listenQuantityChange() {
        this.$scope.on('click', '.increments-quickShop [data-quantity-change] button', event => {
            const $target = $(event.target);
            const itemId = $target.data('item-id');
            const viewModel = this.getViewModel(this.$scope);
            const $el = viewModel.quantity.$input;
            ...
            // update hidden input
            viewModel.quantity.$input.val(newQty);
            // update text
            viewModel.quantity.$text.text(newQty);
        });
    ...
    showMessageBox(message) {
        const $messageBox = $('.quickShopPopup .productAttributes-message');

        if (message) {
            if (message.includes('selected product combination is currently unavailable')) {
                message = message = this.context.ProductVariantSoldOutMessage
            }
     ...
     */
    updatePriceView(viewModel, viewModelQS, price, qty) {
    ...
    updateView(data, content = null) {
        const viewModel = this.getViewModel(this.$scope);
        const $scopeQS = $('.productView-price',this.$scope);
        const viewModelQS = this.getViewModel($scopeQS);
        ...
        if (_.isObject(data.price)) {
            this.updatePriceView(viewModel, viewModelQS, data.price, $qty);
        }
